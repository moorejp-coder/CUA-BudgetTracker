  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {nudges.map((n) => {
          const meta = EVENT_LABELS[n.event_type] ?? { label: n.event_type, tone: "bg-surface-sunken text-white/60" };
          return (
            <div key={n.id} className="card flex items-start justify-between gap-4">
              <div className="flex-1">
                ...
                <p className="text-sm text-white/80 mt-2">{n.message}</p>
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-[10px] text-white/30 uppercase tracking-wide">{n.source}</span>
                  {meta.link && (
              ...
              <button onClick={() => dismiss(n.id)} className="text-white/30 hover:text-white text-xs shrink-0">
                Dismiss
              </button>
            </div>
          );
