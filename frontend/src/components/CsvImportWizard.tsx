    ...
    <div className="card max-w-2xl">
          ...
          <div key={label} className={`flex items-center gap-2 ${step === i + 1 ? "text-accent" : ""}`}>
            <span
              className={`w-5 h-5 rounded-full flex items-center justify-center text-[11px] ${
                step === i + 1 ? "bg-accent text-white" : "bg-surface-sunken"
              }`}
            >
              {i + 1}
            </span>
        ...
        <div className="space-y-4">
          ...
          <div className="overflow-x-auto border border-border-subtle rounded-lg">
            <table className="w-full text-xs">
              <thead className="bg-surface-sunken text-white/50">
                <tr>
              ...
              <tbody>
                {preview.sample_rows.map((row, i) => (
                  <tr key={i} className="border-t border-border-subtle">
                    {INTERNAL_FIELDS.map((f) => (
                      <td key={f} className="px-3 py-2 text-white/70">
      ...
      {step === 3 && result && (
        <div className="space-y-3">
          <div className="text-income font-semibold">Imported {result.imported} transactions.</div>
          {result.duplicates_skipped > 0 && (
            <div className="text-white/60 text-sm">{result.duplicates_skipped} duplicates skipped.</div>
          )}
          {result.errors.length > 0 && (
            <div className="text-warning text-sm">
              {result.errors.length} rows had errors:
              <ul className="list-disc list-inside mt-1 text-white/50">
